<script lang="ts">
  ...
  const DRAW_STORAGE_KEY = 'drawPageDrawingData';
...
*/
  ...
  function clearCanvas() {
    ...
    sessionStorage.removeItem(DRAW_STORAGE_KEY);
  }
  ...
  // Save drawing content to sessionStorage
  function saveDrawingState() {
    try {
      const data = JSON.stringify(drawingContent);
      sessionStorage.setItem(DRAW_STORAGE_KEY, data);
      console.log('Draw page state saved');
    } catch (err) {
      console.error('Error saving draw state', err);
    }
  }

  // Load drawing content from sessionStorage
  function loadDrawingState() {
    try {
      const saved = sessionStorage.getItem(DRAW_STORAGE_KEY);
      if (saved) {
        drawingContent = JSON.parse(saved);
        // ensure strokes array exists
        if (!drawingContent.strokes) drawingContent.strokes = [];
        // Re-render to canvas
        renderStrokes();
        console.log('Draw page state loaded');
      }
    } catch (err) {
  ...
  // Save before navigate and unload
  if (typeof window !== 'undefined') {
    beforeNavigate(() => {
      saveDrawingState();
    });
    window.addEventListener('beforeunload', saveDrawingState);
  }
  onDestroy(() => {
    saveDrawingState();
  });

  // integrate save calls in drawing modifications
</script>
